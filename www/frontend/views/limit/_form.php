...
<body style="background-color: #41DBC6;background-size: cover;background-position: center;">
    <div class="">
        <div class="site-signup">
            ...
            <?php 
            // ดึงข้อมูล limit ที่มี create_by เป็นคนนั้น
            $limitCount = \app\models\Limit::find()->where(['create_by' => Yii::$app->user->identity->id])->sum('amount');

            // ตรวจสอบว่ามีข้อมูลในฐานข้อมูลหรือไม่ ถ้ามีแสดงข้อความ "This month's limit has been added." แต่ถ้าไม่มีก็แสดงปุ่ม Save
            if ($limitCount > 0) {
                echo 'This month\'s limit has been added.';
            } else {
                echo Html::submitButton('Save', ['class' => 'btn btn-success btn-user btn-block rounded-pill shadow-lg']);
            }
            ?>
